...
module.exports = {
    ...
    // Task 2
    deleteManyMovies: function(req, res){

        let actorId = req.params.id; 
       Actor.findOneAndDelete(actorId).exec(function(err,actor){
           for (i = 0; i<actor.movies.length;i++){
                Movie.findByIdAndDelete(actor.movies[i]).exec(function(err,data){
                    if (err) return res.status(400).json(err);
                    console.log(data);
